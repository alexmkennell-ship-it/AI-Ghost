// bob.js — Expressive Build 2.8 (Smart Cache)
// • Universal smooth transitions + pose carryover
// • Sleep dolly-out / wake dolly-in (never closer than 1.8m)
// • Guaranteed stand-up on wake (shrug → idle)
// • Raspy ONYX voice, mic-lock, amplitude jaw/fingers, emotion eyes
// • 3-hour chat memory cache (localStorage) + mood/camera persistence
...
const CACHE_KEYS = {
  CHAT:   "bob_chat",
  MOOD:   "bob_mood",
  ORBIT:  "bob_camera_orbit",
  LASTTS: "bob_last_talk_ts"
};
const MAX_CACHE = 10;
...
function loadJSON(key, fallback){
  try {
    ...
    // legacy format - just purge
    return fallback;
  } catch { return fallback; }
}
function saveJSON(key, value){
  try { localStorage.setItem(key, JSON.stringify({ ts: now(), value })); } catch {}
}

function getChatHistory(){
  const list = loadJSON(CACHE_KEYS.CHAT, []);
  return Array.isArray(list) ? list : [];
}
function saveChat(user, bob){
  const list = getChatHistory();
  list.push({ user, bob, ts: now() });
  while (list.length > MAX_CACHE) list.shift();
  saveJSON(CACHE_KEYS.CHAT, list);
}
function rememberMood(m){ saveJSON(CACHE_KEYS.MOOD, m); }
function recallMood(){ return loadJSON(CACHE_KEYS.MOOD, "calm"); }
function rememberOrbit(orbit){ saveJSON(CACHE_KEYS.ORBIT, orbit); }
function recallOrbit(){ return loadJSON(CACHE_KEYS.ORBIT, "0deg 75deg 1.8m"); }
function stampTalk(){ saveJSON(CACHE_KEYS.LASTTS, now()); }
function recentTalkWithin(ms){ const t = loadJSON(CACHE_KEYS.LASTTS, 0); return now()-t < ms; }
...
async function smoothCameraTransition(targetRadius, duration = 1000){
  ...
  const step = (now)=>{
    ...
    const radius = lerp(start, end, eased);
    const out = `0deg 75deg ${radius.toFixed(2)}m`;
...
async function ensureGlbUrl(name){
  ...
  const p = (async ()=>{
    ...
    if (!res.ok) throw new Error(`Fetch fail ${name}`);
...
function startAmplitudeDriveFor(audio){
  ...
  try{
    ...
    const data = new Uint8Array(analyser.fftSize), jawS={v:0}, fingerS={v:0};
    const drive = ()=>{
      analyser.getByteTimeDomainData(data);
      let sum=0; for(let i=0;i<data.length;i++){ const v=(data[i]-128)/128; sum+=v*v; }
      const rms=Math.sqrt(sum/data.length), amp=clamp(rms*6,0,1);
      jawS.v = jawS.v*0.7 + amp*0.3; fingerS.v = fingerS.v*0.8 + amp*0.2;
      ensureBindings();
...
// ---------------- Animation Core (universal smoothing) ----------------
...
function startMicroIdle(){
  if (microIdleRAF) return; microIdleActive=true; const phase=Math.random()*Math.PI*2;
  const tick=()=>{
...
// ---------------- Speaking (AI+TTS with history) ----------------
async function speakAndAnimate(userText){
  if (!userText) return;
  try{
    state="talking"; stopMicroIdle(); setStatus("💬 Thinking...");

    // Pull last few exchanges (3-hour TTL) and mood
    const history = getChatHistory();
    const mood = recallMood();

    // Chat (send prompt + history + mood hint; worker can ignore if unused)
    ...
    audio.onended = async ()=>{
      stopAmplitudeDrive();
      stampTalk();
      saveChat(userText, reply); // persist chat pair (3-hour TTL)

      state="idle"; setStatus("👂 Listening...");
      const mood = recallMood();
      const nextIdle = (mood==="sleepy" && Math.random()<0.4) ? ANIM.YAWN : pick(idleVariety);
      await setAnim(nextIdle, { minHoldMs:600, blendMs:300 });
...
function bumpActivity(){ lastActivity = now(); }

async function maybeSleep(){
  if (state!=="idle" || sleepLock) return;
  // Don’t sleep too soon after user just spoke
  if (recentTalkWithin(15000)) return;

  if (Math.random() < 0.25){
    sleepLock=true; state="sleeping"; stopMicroIdle();
    setStatus("😴 Nodding off...");
    await smoothCameraTransition(3.2, 1200); // dolly back smoothly
    await setAnim(ANIM.SLEEP, { minHoldMs:1800, blendMs:600 });
    // stay sleeping until wake (voice/click)
  }
}
setInterval(async ()=>{ if (state==="idle" && now()-lastActivity>40000) await maybeSleep(); }, 1000);

// ---------------- Wake Helpers ----------------
async function standUpSequence(){
  await smoothCameraTransition(1.8, 1000);        // glide back in (never < 1.8)
  await setAnim(ANIM.SHRUG,     { minHoldMs:400, blendMs:700 });
  await setAnim(ANIM.IDLE_MAIN, { minHoldMs:800, blendMs:600 });
  startMicroIdle(); state="idle"; sleepLock=false; setStatus("👂 Listening...");
}
...
if (window.SpeechRecognition){
  ...
  // initial user gesture to unlock mic/autoplay
...
async function boot(){
  try{
    ...
    // Initialize camera immediately to avoid warmup hang and snap
    setCameraOrbitImmediate(recallOrbit()); // restore last orbit if available
    activeMV.setAttribute("camera-target", "0m 1.2m 0m"); // chest-ish focal point

    setStatus("Warming up…"); await warmup(); console.log("✅ Warmup complete");

    await setAnim(ANIM.IDLE_MAIN, { minHoldMs:800, blendMs:300 });
    state="idle"; lastActivity=now(); startMicroIdle();

    // ensure we never start closer than 1.8m
    const orbit = activeMV.getAttribute("camera-orbit") || "0deg 75deg 1.8m";
    const parts = orbit.split(" ");
    const r = parseFloat(parts[2])||1.8;
