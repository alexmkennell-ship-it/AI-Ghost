// bob.js — Expressive Build 2.7
// Smooth camera dolly (never zooms closer), universal pose blending,
// guaranteed stand-up on wake, raspy voice, emotion eyes, jaw/finger amplitude drive.
...
async function smoothCameraTransition(targetRadius, duration=1000){
  ...
  const start = parseFloat((mv.getAttribute("camera-orbit")||"0deg 75deg 1.8m").split(" ")[2]) || 1.8;
  const end = clamp(targetRadius,1.8,3.5);
  const startTime = performance.now();
  const step = (now)=>{
    const t=Math.min((now-startTime)/duration,1);
    const eased=0.5-0.5*Math.cos(Math.PI*t);
    const radius=lerp(start,end,eased);
    mv.setAttribute("camera-orbit",`0deg 75deg ${radius.toFixed(2)}m`);
...
function ensureBindings(){
  ...
  if(!boneSearchDone){
    ...
    jawBone=jaw;fingerBones=fingers.slice(0,6);
    boneSearchDone=true;
  }
...
function setEmotionEyesFromText(t){
  ...
  else if(/mischief|trick|sneak/.test(x)){c={r:.95,g:.5,b:1};i=.9;}
...
function startAmplitudeDriveFor(audio){
  ...
  try{
    ...
    const drive=()=>{
      ...
      jS.v=jS.v*.7+amp*.3;fS.v=fS.v*.8+amp*.2;
      ensureBindings();
...
function applyPose(mv,p){const s=getThreeScene(mv);if(!s)return;s.traverse?.(o=>{const t=p[o.name];if(t){o.rotation.copy(t.r);o.position.copy(t.p);}});}
...
async function speakAndAnimate(userText){
  ...
  }catch(e){console.error("Speech error:",e);stopAmplitudeDrive();state="idle";micLocked=false;startMicroIdle();}
}
...
document.addEventListener("pointerdown",()=>{bumpActivity();if(state==="sleeping"){state="waking";standUpSequence();}},{passive:true});
...
if(window.SpeechRecognition){
  ...
  rec.onresult=async e=>{
    const t=e.results[e.results.length-1][0].transcript.trim().toLowerCase();if(!t)return;
    console.log("🎤 Heard:",t);
...
async function boot(){
  try{
    ...
    state="idle";lastActivity=Date.now();startMicroIdle();
    activeMV.setAttribute("camera-orbit","0deg 75deg 1.8m");
